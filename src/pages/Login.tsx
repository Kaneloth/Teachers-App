...
export default function Login() {
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      // Always land on /onboarding — it detects existing users and skips
      // to /home automatically, so both new and returning users are handled.
        ...
        <form onSubmit={handleVerifyEmail} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input id="emailOtp" value={emailOtp} onChange={e => setEmailOtp(e.target.value.replace(/\D/g, ''))}
              placeholder="123456" className="rounded-xl text-center text-2xl tracking-[0.4em] font-mono"
