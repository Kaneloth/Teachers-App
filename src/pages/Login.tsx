...
export default function Login() {
  ...
  // Biometric state – exactly like Base44
  const [loginMethod, setLoginMethod] = useState(() => localStorage.getItem('loginMethod') || 'password');
  const [biometricLoading, setBiometricLoading] = useState(false);
  const [accountBlocked, setAccountBlocked] = useState(null); // optional

  // Store the access token after password login (like Base44's base44_access_token)
  const storeAccessToken = async () => {
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.access_token) {
      localStorage.setItem('crosssa_access_token', session.access_token);
      localStorage.setItem('loginMethod', 'biometric');
    }
  };

  // Restore session from stored access token
  const restoreFromAccessToken = async (): Promise<boolean> => {
    const token = localStorage.getItem('crosssa_access_token');
    if (!token) return false;
    try {
      // Set the session using only the access token (refresh token empty)
  ...
  // ── Biometric login (identical to Base44's pattern) ─────────
  const handleBiometricLogin = async () => {
    setBiometricLoading(true);
    try {
      // 1. WebAuthn verification
      const storedId = localStorage.getItem('biometricCredentialId');
      if (!storedId) throw new Error('No biometric credential found. Please log in with password first.');
      ...
      // 2. Restore session from stored access token
      const success = await restoreFromAccessToken();
      if (success) {
        navigate('/home');
      } else {
        // Token expired or invalid – fall back to password, keep credential for next time
  ...
  // ── Password login (stores access token for future biometric) ──
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Store access token for biometric (like Base44)
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <div className="text-center space-y-1">
            <p className="text-base font-semibold text-foreground">{biometricLoading ? 'Verifying...' : 'Tap to sign in with Biometric'}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2 text-sm text-muted-foreground">
        ...
        <p>Don't have an account? <Link to="/register" className="text-primary font-semibold hover:underline">Create one</Link></p>
      </div>
    </div>
  );
}