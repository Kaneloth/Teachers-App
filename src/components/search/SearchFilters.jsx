...
export default function SearchFilters({ filters, onFiltersChange }) {
  ...
  const handleReset = () => {
    const reset = { province: '', subject: '', phase: '', activeOnly: false };
    setLocal(reset);
  ...
  const activeCount = [
    local.province, local.subject, local.phase, local.activeOnly
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="sm" className="gap-2 rounded-full border-border">
          ...
          Filters
          {activeCount > 0 && (
            <Badge className="bg-primary text-primary-foreground text-[10px] px-1.5 py-0 min-w-[18px]">
              {activeCount}
            </Badge>
          )}
        </Button>
          ...
          <div className="flex items-center justify-between py-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Show only educators seeking transfers
              </p>
            </div>
            <Switch
              checked={local.activeOnly}
              onCheckedChange={(v) => setLocal(p => ({ ...p, activeOnly: v }))}
            />
          </div>

          <Button onClick={handleApply} className="w-full rounded-xl h-12 text-base font-semibold">
            Apply Filters
          </Button>
        </div>
      </SheetContent>
