...
export default function CVStepEducation({ data, onChange }) {
      ...
      {data.map((entry, i) => (
        <div key={i} className="bg-card rounded-2xl border border-border p-4 space-y-3">
          ...
          <Field label="Institution">
            <Input value={entry.institution} onChange={e => set(i, 'institution', e.target.value)} placeholder="e.g. UNISA" className="rounded-xl" />
          </Field>
          <Field label="Qualification">
            <Input value={entry.qualification} onChange={e => set(i, 'qualification', e.target.value)} placeholder="e.g. B.Ed (FET)" className="rounded-xl" />
          </Field>
          <Field label="Year Completed">
            <Input value={entry.year} onChange={e => set(i, 'year', e.target.value)} placeholder="e.g. 2018" className="rounded-xl" />
          </Field>
        </div>
      ))}
