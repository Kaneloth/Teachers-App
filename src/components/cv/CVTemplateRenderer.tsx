      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', lineHeight: 1 }}>
        {icon && (
          <span style={{
            ...
            position: 'relative',
            top: '-3px', // shift icon up to align with text baseline
          }}>
            {icon}
          </span>
        )}
...
function MinimalTemplate({ data, wrapperStyle, validEdu, validExp }: any) {
  const { personal, skills } = data;
  const validRefs = (data.references || []).filter((r: any) => r.name);
  return (
    <>
      <div style={wrapperStyle}>
      ...
      {validRefs.length > 0 && (
        <div style={{ pageBreakBefore: 'always', breakBefore: 'page', padding: '40px 44px', background: '#fff' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '14px 28px' }}>
              {validRefs.map((r: any, i: number) => (
                <div key={i}>
                  ...
                  {r.organisation && <div style={{ color: '#6b7280', fontSize: '12px' }}>{r.organisation}</div>}
                  {r.relationship && <div style={{ color: '#6b7280', fontSize: '11px', fontStyle: 'italic' }}>{r.relationship}</div>}
                  {(r.phone || r.email) && <div style={{ color: '#6b7280', fontSize: '11px', marginTop: '3px' }}>{[r.phone, r.email].filter(Boolean).join(' · ')}</div>}
                </div>
              ))}
            </div>
          </MinimalSection>
