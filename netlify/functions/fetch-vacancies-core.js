/**
 * fetch-vacancies-core.js — shared fetch/upsert/cleanup logic, used by:
 *   - fetch-vacancies.js           (POST, manually triggered by any logged-in user)
 *   - vacancies-refresh-daily.js   (Netlify scheduled function, runs automatically)
 *
 * Same split as match-scan.js / match-scan-daily.js / match-scan-core.js
 * elsewhere in this repo, for the same reason: a scheduled function can't
 * also be called directly over HTTP in production, so the actual logic
 * lives here where both entry points can share it.
 *
 * v5 changes vs v4 (unchanged from the original file):
 ...
 *   - Careers24 now fetches from 5 category pages (education + others).
 *   - Education filter removed from Careers24 — all valid jobs are kept.
 *
 * v6 changes (this pass):
 *   - Added cleanupStaleVacancies(): neither Adzuna's API nor our
 *     lightweight Careers24 listing-page scrape actually expose an
 *     application closing date (see the comment on cleanupStaleVacancies
 *     below for why that's not practically fixable), so every scraped row
 *     always had closing_date: null. Combined with the upsert's
 *     ignore-duplicates behavior, that meant nothing was ever removed —
 *     vacancies just accumulated forever. Cleanup now runs after every
 *     fetch: rows with a real closing_date are removed once that date has
 *     passed; rows without one (currently all of them) are removed once
 *     they're older than VACANCY_TTL_DAYS since first seen (created_at,
 *     which — since duplicates are ignored, not updated — reliably stays
 *     the original first-seen date).
 *   - Extracted runVacanciesRefresh() as the single orchestrator so both
 *     entry points get fetch + upsert + cleanup automatically, in the
 *     same order, with the same summary shape.
 *
 * Env vars:
...
// How long a scraped listing (no real closing_date available) stays
// before it's treated as stale and removed. 30 days is a common default
// for general job-posting freshness — adjust here if you want a
// different window. Must match the constant of the same purpose in
// VacanciesPage.tsx (VACANCY_TTL_DAYS), which applies the same rule
// client-side as an immediate safety net between cleanup runs.
...
/* ─── Cleanup: remove expired / stale listings ──────────────────────────────
 * Real closing dates aren't available to capture in the first place:
 *   - Adzuna's public API doesn't return an application-deadline field at
 *     all — it's simply not part of the data they expose, regardless of
 *     whether the original employer posting had one.
 *   - Our Careers24 scraper only reads listing pages (title + link) for
 *     speed — fetching each individual job's detail page (where a real
 *     closing date might appear) would mean 50-250+ additional HTTP
 *     requests per run, which blows well past this function's timeout.
 * So for every row currently in this table, closing_date is null. The
 * fallback here is a TTL against created_at, which (since the upsert
 * ignores duplicates rather than updating them) reliably holds the date
 * we first saw the listing. Any row that DOES have a real closing_date
 * (e.g. a school-posted vacancy from elsewhere in the app) is still
 * removed on its actual expiry instead of the TTL guess. */
async function cleanupStaleVacancies(log) {
  const cutoffIso = new Date(Date.now() - VACANCY_TTL_DAYS * 24 * 60 * 60 * 1000).toISOString();
  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD — matches closing_date's date format

  const filter =
    `or=(and(closing_date.not.is.null,closing_date.lt.${today}),` +
    `and(closing_date.is.null,created_at.lt.${cutoffIso}))`;

  const res = await fetch(`${process.env.VITE_SUPABASE_URL}/rest/v1/vacancies?${filter}`, {
    method: 'DELETE',
    headers: {
      apikey: process.env.VITE_SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${process.env.VITE_SUPABASE_SERVICE_ROLE_KEY}`,
  ...
  if (!res.ok) {
    log.push(`Cleanup failed (${res.status}): ${await res.text()}`);
    return 0;
  }

  const deleted = await res.json().catch(() => []);
  const count = Array.isArray(deleted) ? deleted.length : 0;
  log.push(`Cleanup: removed ${count} stale/expired listing${count === 1 ? '' : 's'} ` +
    `(past a real closing_date, or no closing_date and older than ${VACANCY_TTL_DAYS} days)`);
  return count;
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   ORCHESTRATOR — fetch from all sources, upsert, then clean up stale rows.
   Used by both fetch-vacancies.js (manual) and vacancies-refresh-daily.js
   (scheduled).
   ═══════════════════════════════════════════════════════════════════════════ */
export async function runVacanciesRefresh() {
  const log = [];
  ...
  const removed = await cleanupStaleVacancies(log);
